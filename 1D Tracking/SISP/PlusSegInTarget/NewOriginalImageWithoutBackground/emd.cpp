...
{
  int dX = F1->X - F2->X;
  return abs(dX);//sqrt((float)(abs(dX)*abs(dX) + abs(dY)*abs(dY) + abs(dZ)*abs(dZ))); 
}
	  ...
	  flow_t *Flow, int *FlowSize)
{
  ...
  node1_t* U;
  node1_t* V;
  U = new node1_t [MAX_SIG_SIZE1];
  V = new node1_t [MAX_SIG_SIZE1];
  //printf("here1");
...
{    
  ...
  //deltaMin = AINFINITY;
  deltaMin = _C[0][0] - U[0].val - V[0].val + 1;
  for(i=0; i < _n1; i++)
    for(j=0; j < _n2; j++)
      if (! _IsX[i][j])
	{
...
static void newSol()
{
	...
	node2_t ** Loop;
	Loop = new node2_t* [2*MAX_SIG_SIZE1];
...
static int findLoop(node2_t **Loop)
{
  ...
  char* IsUsed;
  IsUsed = new char [2*MAX_SIG_SIZE1];
...
//  printf("\nHERE\n");
  ...
  double** Delta;
  Delta = new double* [MAX_SIG_SIZE1];
  for(i = 0; i < MAX_SIG_SIZE1; i++)Delta[i] = new double [MAX_SIG_SIZE1];
  
  //node1_t Ur[MAX_SIG_SIZE1], Vr[MAX_SIG_SIZE1];
  node1_t* Ur;
  node1_t* Vr;
  Ur = new node1_t [MAX_SIG_SIZE1];
  Vr = new node1_t [MAX_SIG_SIZE1];
  ...
  do
	...
	printf("[%d]",CurV-Vr);
      ...
      found = 0; 
      deltaMin = AINFINITY;      
	  //deltaMin = Delta[uHead.Next->i][vHead.Next->i] + 1;
      PrevU = &uHead;
      for (CurU=uHead.Next; CurU != NULL; CurU=CurU->Next)
	{
	  int i;
	...
	delete [] Ur;
	delete [] Vr;
	for(i = 0; i < MAX_SIG_SIZE1; i++)delete [] Delta[i];
	delete [] Delta;
}
