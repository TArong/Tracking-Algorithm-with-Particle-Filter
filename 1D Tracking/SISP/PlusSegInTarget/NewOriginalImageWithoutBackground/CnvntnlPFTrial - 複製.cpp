...
#define BlockSize 1
#define Percent BlockSize*BlockSize*0.5
...
//	map<int,int> area;
   ...
   long long int DistanceY, DistanceU, DistanceV;
               ...
               flag_V_w[k] =0;
 	  			...
 	  			for(k=max(0,nowtracker->PPostion[i][1]-nowtracker->PBoxWH[i][1]); k<= min(FrameH-1, nowtracker->PPostion[i][1] + nowtracker->PBoxWH[i][1]); k++){
 	  				for(l=max(0, nowtracker->PPostion[i][0]-nowtracker->PBoxWH[i][0]); l< min(FrameW-1, nowtracker->PPostion[i][0] + nowtracker->PBoxWH[i][0]); l+=2){
						...
						TempHistValue, TempHistValue, flag_Y_w, flag_U_w, flag_V_w, HistogramCandidate_Y_E, HistogramCandidate_Y_O, HistogramCandidate_U_E, HistogramCandidate_U_O, HistogramCandidate_V_E, HistogramCandidate_V_O, 1, 1);
...
//            fouttest3 << "frame: " << f << endl;
               ...
               distance_compute(HistogramCandidate_Y_E, HistogramCandidate_Y_O, nowtracker->HistogramTarget_Y_E, nowtracker->HistogramTarget_Y_O, DistanceY, 4, TotalHistogramValue, nowtracker->totalhistogram );
               //cout << DistanceY << endl;
               //if(f == 653)system("pause")	;
               distance_compute(HistogramCandidate_U_E, HistogramCandidate_U_O, nowtracker->HistogramTarget_U_E, nowtracker->HistogramTarget_U_O, DistanceU, 4, TotalHistogramValue, nowtracker->totalhistogram );
               //system("pause")	;
               //system("pause")	;
               distance_compute(HistogramCandidate_V_E, HistogramCandidate_V_O, nowtracker->HistogramTarget_V_E, nowtracker->HistogramTarget_V_O, DistanceV, 4, TotalHistogramValue, nowtracker->totalhistogram );
               
               
               Distance = (DistanceY*DistanceY + DistanceU*DistanceU/32 + DistanceV*DistanceV/32) / TotalHistogramValue / nowtracker->totalhistogram/ TotalHistogramValue / nowtracker->totalhistogram;// * 10000 * 1024 * 21.5;
				}
				time2 = clock();
...
// 	  		clock1 = clock();
			...
			for(i=0;i<NumParticleonObject;++i){
				...
				if(nowtracker->ratio > 1){
					if(tempG < 12) tempG = 12;
					if(tempH < 12*nowtracker->ratio) tempH = 12*nowtracker->ratio;
				}
				else{
					if(tempG < 12*nowtracker->ratio) tempG = 12*nowtracker->ratio;
...
//            fouttest3 << "frame: " << f << endl;
               ...
               distance_compute(HistogramCandidate_U_E, HistogramCandidate_U_O, nowtracker->HistogramTarget_U_E, nowtracker->HistogramTarget_U_O, DistanceU, 4, TotalHistogramValue, nowtracker->totalhistogram);
               distance_compute(HistogramCandidate_V_E, HistogramCandidate_V_O, nowtracker->HistogramTarget_V_E, nowtracker->HistogramTarget_V_O, DistanceV, 4, TotalHistogramValue, nowtracker->totalhistogram);
               Distance = (DistanceY*DistanceY + DistanceU*DistanceU/32 + DistanceV*DistanceV/32) / TotalHistogramValue / nowtracker->totalhistogram / TotalHistogramValue / nowtracker->totalhistogram ;
				}
			...
			if(nowtracker->confidence >ConfidenceThreshold && !nowtracker->deleteTracker){//||( (nowtracker->ObjectPX > LeftThreshold && nowtracker->ObjectPX < RightThreshold) && (nowtracker->ObjectPY > TopThreshold && nowtracker->ObjectPY < BottomThreshold))){	
				...
				tempD = min((int)FrameW-1, (int)(nowtracker->ObjectPX + 1.2*nowtracker->ObjectBoxW));
 	  			for(k=(tempA);k<=(tempB); ++k){
					if((tempC)%2 == 0){
						FrameDisplayY[tempC] = nowtracker->colory;
						FrameDisplayY[tempC + 1] = nowtracker->colory;
					}
					else{
						FrameDisplayY[tempC] = nowtracker->colory;
						FrameDisplayY[tempC - 1] = nowtracker->colory;
					...
					if((tempD)%2 == 0){
	 	  				FrameDisplayY[tempD] = nowtracker->colory;
						FrameDisplayY[tempD + 1] = nowtracker->colory;
					}
					else{
	 	  				FrameDisplayY[tempD] = nowtracker->colory;
						FrameDisplayY[tempD - 1] = nowtracker->colory;
            ...
            for(k = 0; k < 256; k++){
               ...
               flag_V_w[k] =0;
				...
				centroid[1] = (max(0,nowtracker->ObjectPX-nowtracker->ObjectBoxW) + min(FrameW-1, nowtracker->ObjectPX + nowtracker->ObjectBoxW)) / 2;
 	  			for(k=max(0,nowtracker->ObjectPY-nowtracker->ObjectBoxH); k<= min(FrameH-1, nowtracker->ObjectPY + nowtracker->ObjectBoxH); ++k){
 	  				for(l=max(0,nowtracker->ObjectPX-nowtracker->ObjectBoxW); l< min(FrameW-1, nowtracker->ObjectPX + nowtracker->ObjectBoxW); l+=2){
						...
						TotalHistogramValue+=2*TempHistValue;
						BU(framebufferY[k*FrameW + l], framebufferY[k*FrameW + l + 1], framebufferU[k/2*FrameW2+ l/2], framebufferU[k/2*FrameW2+ (l+1)/2], framebufferV[k/2*FrameW2+ l/2] , framebufferV[k/2*FrameW2+ (l+1)/2], 
						TempHistValue, TempHistValue, flag_Y_w, flag_U_w, flag_V_w, LearningHistogram_Y_E, LearningHistogram_Y_O, LearningHistogram_U_E, LearningHistogram_U_O, LearningHistogram_V_E, LearningHistogram_V_O, 1,1);
				...
				for(k = 0; k < 128; k++){
					...
					if(!nowtracker->NoParticle){
						nowtracker->HistogramTarget_Y_E[k] = LearningRatio * (double)LearningHistogram_Y_E[k] + (double)nowtracker->HistogramTarget_Y_E[k] * (1-LearningRatio);//LearningHistogram_Y_E[k];
						nowtracker->HistogramTarget_Y_O[k] = LearningRatio * (double)LearningHistogram_Y_O[k] + (double)nowtracker->HistogramTarget_Y_O[k] * (1-LearningRatio);//LearningHistogram_Y_O[k];
						nowtracker->HistogramTarget_U_E[k] = LearningRatio * (double)LearningHistogram_U_E[k] + (double)nowtracker->HistogramTarget_U_E[k] * (1-LearningRatio);//LearningHistogram_U_E[k];
						nowtracker->HistogramTarget_U_O[k] = LearningRatio * (double)LearningHistogram_U_O[k] + (double)nowtracker->HistogramTarget_U_O[k] * (1-LearningRatio);//LearningHistogram_U_O[k];
						nowtracker->HistogramTarget_V_E[k] = LearningRatio * (double)LearningHistogram_V_E[k] + (double)nowtracker->HistogramTarget_V_E[k] * (1-LearningRatio);//LearningHistogram_V_E[k];
						nowtracker->HistogramTarget_V_O[k] = LearningRatio * (double)LearningHistogram_V_O[k] + (double)nowtracker->HistogramTarget_V_O[k] * (1-LearningRatio);//LearningHistogram_V_O[k];
                  ...
                  flag_V_w[k] =0;
					...
					for(k=max(0,newtrackernode->IniPy-newtrackernode->IniHalfH); k<= min(FrameH-1, newtrackernode->IniPy + newtrackernode->IniHalfH); ++k){
                     ...
                     //newtrackernode->totalhistogram+= (segbufferY[k*FrameW + l] !=0 ? 1 : 0) + ( segbufferY[k*FrameW + l + 1] != 0 ? 1 : 0);
							...
							TempHistValue, TempHistValue, flag_Y_w, flag_U_w, flag_V_w, newtrackernode->HistogramTarget_Y_E, newtrackernode->HistogramTarget_Y_O, newtrackernode->HistogramTarget_U_E, newtrackernode->HistogramTarget_U_O, newtrackernode->HistogramTarget_V_E, newtrackernode->HistogramTarget_V_O, 1, 1);
 	  						}
